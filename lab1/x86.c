...
run(process_t *proc)
{
	...
	// 把system call时push到kernal
	// stack上的各种register再取出来，恢复现场，让process继续运行，这也是一种context
	// switch(从内核态到用户态)
	asm volatile("movl %0,%%esp\n\t"
		     ...
		     "popl %%ds\n\t"
		     "addl $8, %%esp\n\t"
		     // iret是processor的指令，叫Interrupt Return,
		     // 作用是从kernal stack上pop出进入system call
		     // 时push到stack上的各种register
		     // 参考：http://www.fermimn.gov.it/linux/quarta/x86/iret.htm
		     "iret"
