...
start(void)
{
	...
	for (i = 0; i < NPROCS; i++) {
		...
		// init the process's wait queue
		memset(proc_array[i].p_wait_queue, 0 ,
		sizeof(proc_array[i].p_wait_queue));
	}
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_GETPID:
		...
		// surprisingly simple:
		// get_pid的执行结果放在eax寄存器中，等待应用程序去取
		// 这是一个约定，参考process.h中对应的system call
	...
	case INT_SYS_EXIT:
		...
		// 保存应用程序的退出码(在看代码之前，心里默念3遍：“这是内核代码”
		// 有助于理解
		current->p_exit_status = current->p_registers.reg_eax;
		int i;
		// iterate current process's wait queue to wake them up
		// 适用于子进程尚未结束，父进程开始wait的情况
		for (i = 1; i < NPROCS; i++) {
			if (current->p_wait_queue[i]) {
				proc_array[i].p_state = P_RUNNABLE;
				// wait调用的返回值放在eax寄存器中
				// 注意处理：对于已经是P_EMPTY状态的process，wait
				// 应该返回错误-1。相当于把下面INT_SYS_WAIT的部分
				// 逻辑搬到这里来
				proc_array[i].p_registers.reg_eax =
				current->p_state == P_EMPTY ? -1: current->p_exit_status;
				// 把坑空出来，给别人一点机会
	...
	case INT_SYS_WAIT: {
		...
		{
			// if the waited process hasn't exit yet, block
			// the waiting process
			current->p_state = P_BLOCKED;
			// add the waiting process to the waited
			// process's wait queue
			proc_array[p].p_wait_queue[current->p_pid] = 1;
		}
		schedule();
	}
...
do_fork(process_t *parent)
{
	...
	//
	// 1. find a empty process descriptor
	pid_t i;
	for (i = 1; i < NPROCS; i++) {
	...
	// no empty process descriptor
	if (i == NPROCS) {
		return -1;
	}
	// 2. copy parent's registers and stack to child
